...
export function loadConfig() {
    ...
    try {
        ...
        throw new Error('No broker config found. Set SEKS_BROKER_URL + SEKS_BROKER_TOKEN or configure ~/.openclaw/openclaw.json');
    }
    const json = JSON.parse(raw);
    const seks = json['seks'];
    const broker = seks?.['broker'];
    if (!broker) {
        throw new Error('No seks.broker section in ~/.openclaw/openclaw.json');
    }
    // Primary/secondary pattern
    const primary = broker['primary'];
    if (primary?.url && (primary.token || primary.tokenCommand)) {
        ...
        const secondary = broker['secondary'];
