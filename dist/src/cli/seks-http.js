...
function parseArgs(argv) {
    ...
    const url = argv[1];
    const opts = { method: method.toUpperCase(), url, headers: {}, headerSecrets: [], timeout: 30 };
    let i = 2;
    while (i < argv.length) {
        const arg = argv[i];
        switch (arg) {
            ...
            case '--auth-basic-user':
                opts.authBasicUser = argv[++i];
                break;
            case '--auth-basic-pass':
                opts.authBasicPass = argv[++i];
                break;
            case '--capability':
                opts.capability = argv[++i];
                break;
            case '--header': {
                const h = argv[++i];
            ...
            case '--header-secret': {
                ...
                const colon = h.indexOf(':');
                if (colon < 1) {
                    console.error(`Invalid header-secret: ${h}`);
                    process.exit(1);
                }
...
//# sourceMappingURL=seks-http.js.map